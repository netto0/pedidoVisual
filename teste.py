...
razoes2 =  """"0 | Adriano Braga dos Santos","1 | Antônio Carlos Amaral","2 | Gilvan Delfino de Oliveira",
          "3 | Ilma Delfino de Oliveira",          "4 | João Ferreira dos Santos","5 | José dos Reis Silva",
          "6 | Lindraci Mendes Damascena","7 | Renato Moura Trindade",         "8 | Rodrigues & Pinheiro Ltda",
          "9 | Santos & Souza Refeições Ltda","10| Pérola do Mucuri Sup. Dist. Alim.",
          "11| Terezinha Joaquina Silva & Cia","12| Hélia Lacerda Machado","13| M. dos Santos Pereira & Cia Ltda",
...
"""for l in razoes:
    if re.findall(r'berto',l) != []:
        print(razoes.index(l))
    else:
        None"""
...
def search(nome, lista):
    to_ascii(lista)
    for l in lista:
        if re.findall(rf'{nome}',l,flags=re.I) != []:
            print(razoes.index(l))
        else:
            None

#print(re.findall(r'Antonio',razoes2, flags=re.))
#search('helcio',razoes)

nome = str(input('Digite um nome para ser pesquisado: '))
search(nome,razoes)
